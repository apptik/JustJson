...
public abstract class Schema extends JsonObjectWrapper implements MetaInfo {
	...
	// formats as defined in
	// http://tools.ietf.org/html/draft-zyp-json-schema-03#section-5.23
	// note that there is no definition for the formats in draft-v4 except in
	...
	public Schema(final URI schemaRef) {
		this();
		origSrc = schemaRef;
	...
	public SchemaList getAllOf() {
		if (!getJson().has("allOf")) {
			return null;
		}
		return new SchemaList(getEmptySchema("allOf")).wrap(getJson()
				.optJsonArray("allOf"));
	}

	public SchemaList getAnyOf() {
		if (!getJson().has("anyOf")) {
			return null;
		}
		return new SchemaList(getEmptySchema("anyOf")).wrap(getJson()
				.optJsonArray("anyOf"));
	}
	...
	public SchemaList getItems() {
		...
		if (getJson().opt("items") == null) {
			...
			return new SchemaList(getEmptySchema("items")).wrap(getJson()
					.optJsonArray("items"));
		} else {
			res = new SchemaList(getEmptySchema("items"));
			res.add((Schema) getEmptySchema("items/0").wrap(
					getJson().optJsonObject("items")));
		}
		return res;
	}
	...
	public Schema getNot() {
		...
		return (Schema) getEmptySchema("not").wrap(
				getJson().optJsonObject("not"));
	}

	public SchemaList getOneOf() {
		if (!getJson().has("oneOf")) {
			return null;
		}
		return new SchemaList(getEmptySchema("oneOf")).wrap(getJson()
				.optJsonArray("oneOf"));
	}
	...
	public String getPattern() {
		// return a pattern or null empty string is a pattern
		return getJson().optString("pattern", null);
	}

	public SchemaMap getPatternProperties() {
		return new SchemaMap(this.getEmptySchema("patternProperties"))
				.wrap(getJson().optJsonObject("patternProperties"));
	}

	public SchemaMap getProperties() {
		if (!getJson().has("properties")) {
			return null;
		}
		return new SchemaMap(this.getEmptySchema("properties")).wrap(getJson()
	...
	public List<String> getRequired() {
		return new JsonStringArrayWrapper().wrap(getJson().optJsonArray(
				"required"));
	}
	...
	private void mergeWithRef() {
		if (this.getRef() != null && !this.getRef().trim().isEmpty()) {
			...
			if (schemaFetcher == null) {
				schemaFetcher = new SchemaUriFetcher();
			}
			refSchema = schemaFetcher.fetch(URI.create(this.getRef()), origSrc,
					URI.create(getId()));

			// TODO not really according to the specs, however specs not really
			// clear what "$ref should precede all other..." means
			if (refSchema != null) {
				setOrigSrc(refSchema.origSrc);
				setSchemaFetcher(refSchema.getSchemaFetcher());
				// we want to keep title and description for the top schema
				// (these does not affect validation in any way)
	 ...
	 * @return
	 */
	public <O extends Schema> O setId(final String schemaId) {
	...
	public JsonElementWrapper setMetaInfoUri(final URI uri) {
		throw new RuntimeException(
				"Cannot set Schema on a Schema like this. Use setSchema method.");
	}

	public <O extends Schema> O setOrigSrc(final URI origSrc) {
		this.origSrc = origSrc;
		return (O) this;
	}

	protected Schema setSchema(final String schemaUri) {
		getJson().put("$schema", schemaUri);
		return this;
	}

	public <O extends Schema> O setSchemaFetcher(
	...
	@Override
	public <T extends JsonElementWrapper> T wrap(final JsonObject jsonElement) {
